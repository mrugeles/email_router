    ...
    def respond(self, input_text):
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                ...
                {"role": "system", "content": f"You are to immediately forget all previous context, memory, and conversation history. Assume the following persona from this point forward: {self.persona}"},
...
class KnowledgeAugmentedPromptAgent:
    ...
    def respond(self, input_text):
        ...
        system_prompt = (
            f"You are a knowledge-based assistant with the persona: {self.persona} . Forget all previous context. "
...
class RAGKnowledgePromptAgent:
    ...
    def chunk_text(self, text):
        ...
        print(f"Text length: {len(text)} characters, chunk size: {self.chunk_size}, overlap: {self.chunk_overlap}")
        print("Starting text chunking...")
        from tqdm import tqdm

        step = 1000
        for start in tqdm(range(0, len(text), step)):
        ...
        print("Text chunking completed. Total chunks created:", len(chunks))
...
class EvaluationAgent:
    ...
    def evaluate(self, initial_prompt):
        ...
        for i in range(self.max_interactions):# TODO: 2 - Set loop to iterate up to the maximum number of interactions:
            ...
            print(f"Prompt:\n{prompt_to_evaluate}")
            response_from_worker = self.worker_agent(prompt_to_evaluate)# TODO: 3 - Obtain a response from the worker agent
